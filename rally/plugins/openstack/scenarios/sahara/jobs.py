...
@validation.required_contexts("users", "sahara_image",
                              "sahara_job_binaries", "sahara_cluster")
@scenario.configure(context={"cleanup": ["sahara"]},
                    name="SaharaJob.create_launch_job_on_cluster")
class CreateLaunchJobOnCluster(utils.SaharaScenario,):

    def run(self, job_type, configs, cluster_id, job_idx=0):
...
@validation.required_contexts("users", "sahara_image",
                              "sahara_job_binaries", "sahara_cluster")
@scenario.configure(context={"cleanup": ["sahara"]},
                    name="SaharaJob.create_launch_jobs_on_diff_clusters")
class CreateLaunchJobsOnDiffClusters(utils.SaharaScenario,):

    def run(self, jobs, cluster_configs):
