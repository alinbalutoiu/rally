...
@scenario.configure(context={"cleanup": ["nova"], "keypair": {},
                             "allow_ssh": {}},
                    name="VMTasks.ovs_iperf_test")
class MaximumOVSThroughput(vm_utils.VMScenario):

    def __init__(self, *args, **kwargs):
        super(MaximumOVSThroughput, self).__init__(*args, **kwargs)

    def run(self, image_ref, image_ref_alt, flavor_ref, win_user, win_pass,
            image_ssh_user, image_ssh_user_alt,
            use_floating_ip=True, floating_network=None, force_delete=False,
            size_to_test=200, time_to_test=30, random_string_size=2,
            retry_count_winrm=120, **kwargs):
        """OVS iperf test
