...
@validation.required_openstack(users=True)
@scenario.configure(context={"cleanup": ["nova"]},
                    name="NovaServers.boot_and_check_console_log_server")
class BootAndCheckConsoleLogServer(utils.NovaScenario,
...
@validation.required_openstack(admin=True, users=True)
@scenario.configure(context={"cleanup": ["nova"]},
                    name=("NovaServers.boot_live_migrate_ssh_server"))
class BootLiveMigrateSshServer(vm_utils.VMScenario, utils.NovaScenario,
                               cinder_utils.CinderScenario):
    def run(self, image, image_ssh_user, flavor, floating_network,
            block_migration=False, disk_over_commit=False, min_sleep=0,
            max_sleep=0, availability_zone=None, host_to_migrate=None,
            **kwargs):
        """Live Migrate a server.
